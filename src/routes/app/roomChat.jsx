...
const AppChat = () => {
    ...
    useEffect(() => {
        const unsubscribe = getMessagesRealTime(roomId, (msgs) => {
            setMessages(
                msgs.map((msg) => ({
                    ...msg,
                    isCreator: msg.userId === auth.user.creatorId,
                }))
            );
        });
        return () => unsubscribe();
    }, [roomId, auth.user.creatorId]);
    ...
    const handleSend = async () => {
        const message = {
            ...
            role: auth.user.role,
            content: input,
            attachmentUrl: attachment ? await uploadAttachment(attachment, roomId, auth.user.uid) : null,
